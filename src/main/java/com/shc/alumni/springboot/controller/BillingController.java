...
public class BillingController {
    ...
    public ResponseEntity<byte[]> downloadBill(@RequestParam String emailAddress) {
        // Fetch the BillPdfEntity from the database using emailAddress
        BillPdfEntity billPdfEntity = billPdfRepository.findByemailAddress(emailAddress)
                .orElseThrow(() -> new RuntimeException("No bill found for email: " + emailAddress));
